...
| **FR-18** | Skill Quiz Bank | Quiz questions are pre-populated for every skill in the database. Each skill has exactly 5 multiple-choice questions (4 options, 1 correct answer). Roadmap generation does not create quizzes; post-node quiz retrieval only reads existing questions from DB by skill_id. This bank is **only** for post-node quizzes; onboarding assessment quizzes are generated by AI per goal and are not stored. |
