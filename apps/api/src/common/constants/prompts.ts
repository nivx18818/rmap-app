...
export const getRoadmapGenerationPrompt = (input: {
  ...
  return [
    ...
    '      "estimatedHours": number (required for every required/optional leaf skill; persisted to roadmap_nodes.estimated_hours),',
    ...
    '- Every required/optional leaf skill node MUST include estimatedHours as a finite positive number. Never omit it and never return null for leaf nodes.',
    '- For leaf estimatedHours, use the catalog defaultEstimatedHours for that exact skill when it is available. If defaultEstimatedHours is null, estimate a realistic completion time for that skill based on difficulty, scope, learner profile, and available time.',
    '- Leaf estimatedHours values must be hours, not days or weeks, and should use at most 2 decimal places.',
