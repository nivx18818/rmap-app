...
| **FR-03** | Personal timeline setup | Users input daily study time (hours) and **desired deadline** (months). During generation, the system validates feasibility using the generated required/optional node estimates against available study hours and warns when the estimate exceeds the timeline by more than 15%.                                       |
| **FR-04** | Skill assessment (Quiz) | The system sends the user's free-text goal to the AI Engine, which infers the role category and returns an onboarding assessment quiz (about 7-10 multiple-choice questions) to gauge user level.                                                                                                                       |
| **FR-05** | Send quiz result to AI  | After the user completes the quiz, the frontend submits the goal, inferred role category, timeline inputs, and quiz answers to the backend. The backend forwards these inputs (plus the role skill map) to the AI Engine to generate the roadmap. Quiz data **is not persisted**; to regenerate, users retake the quiz. |

> **Optimal onboarding flow:** Enter goal + timeline -> Answer AI-generated quiz -> Generate roadmap immediately. Total flow should stay within 3 steps to reduce drop-off.
...
| **ID**    | **Function**                | **Detailed Description**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **FR-06** | AI Roadmap Engine           | Integrate Gemini API. Input: free-text goal, AI-inferred role category, quiz answers, daily study time, and target deadline. The backend also loads the role's skill map from the database and sends it with the user inputs. Output: required skill nodes, priority order, and estimated time per node. AI **must not return a generic roadmap** - it must skip skills the user already has according to the quiz.                                                                                                                                                                                               |
...
| **FR-08** | Prerequisite & unlock logic | Skills maintain a simple prerequisite list (for example, JWT Auth requires HTTP basics and Express routing) for learning guidance and sequencing context. Runtime unlocking follows **spine-sequential rules**: leaves in the first group are unlocked by default; leaves in group N unlock when all required leaves in group N-1 are completed; milestone nodes unlock after the required leaves of the immediately preceding group are completed. GROUP nodes are structural spine sections; they are auto-completed when all required child leaves are completed and cannot be manually completed by the user. |
| **FR-09** | Milestone project           | Each roadmap includes 2-3 predefined **major milestone projects** (for example for Backend: CRUD API -> Auth System -> Deploy Project). When an unlocked milestone is opened, the system lazily generates and stores an AI test suite from the milestone project brief. Each suite contains exactly 6 Node.js-compatible generated tests, uses an 80% pass threshold, and returns per-test pass/fail details with messages for completed submissions. Milestones are highlighted in the roadmap and linked to related skill nodes.                                                                                |
...
| **ID**    | **Function**             | **Detailed Description**                                                                                                                                                                                                                                                                                                                                                     |
| --------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **FR-10** | Skill Tree visualization | Use React Flow to display a tree/graph view. Support zoom and pan. Nodes are color-coded by status (Locked / In Progress / Completed) for authenticated learner views. Anonymous public template previews show the template graph without a roadmap progress tracker. Milestone projects are shown as special nodes, and GROUP nodes are shown as structural spine sections. |
...
| **ID**    | **Function**            | **Detailed Description**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **FR-13** | Update node status      | Authenticated users move eligible leaf node state on personal roadmaps and templates: `Locked -> In Progress -> Completed`. For leaf nodes, transitioning to `Completed` requires `quiz_passed = true` (score >= 60%). Milestone completion is backend-driven only: learners submit a GitHub repository URL, the backend injects the stored AI-generated 6-test suite into the cloned Node.js project, records the overall pass rate plus each generated test's pass/fail result and message, and automatically completes the milestone when pass rate is at least 80%. GROUP nodes are not manually completable; the backend auto-completes them through cascade logic. The system records completion timestamp. |
| **FR-14** | Three-layer progress    | Authenticated roadmap detail displays 3 metrics at once: **(1)** Roadmap completion %, **(2)** Streak - consecutive learning days, **(3)** Skill Readiness for selected goal (calculated from completed important nodes / total important nodes). It also shows completed/total nodes. Template learning progress is stored per user in `user_node_progress`. Anonymous users must not see the roadmap progress tracker. Example: `Roadmap: 40%`                                                                                                                                                                                                                                                                  |
| **FR-15** | Timeline warning        | The system has two timeline checks: generation feasibility compares the generated required/optional node hour estimate with available hours before the deadline; ongoing pace compares actual completed estimated hours with elapsed planned hours after at least one calendar day has passed. Show alert: "You are 20% behind - projected delay is about 5 days" when the relevant deficit is >= 15%.                                                                                                                                                                                                                                                                                                            |
| **FR-16** | Streak & daily activity | The system records learning days (at least one node/task completed). Show streak in the roadmap detail progress summary. Streak-risk reminders are planned for a future notification flow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **ID**    | **Function**    | **Detailed Description**                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **FR-17** | Post-node quiz  | When a user attempts to complete a leaf node (`required` / `optional`), the system shows exactly 5 multiple-choice questions for that skill. If score < 60%: keep the node in progress and show suggestion "You should review this part before continuing." If score >= 60%: set `quiz_passed = true`, automatically mark the node `Completed`, record completion activity, and unlock subsequent learning nodes according to spine-sequential rules. |
| **FR-18** | Skill Quiz Bank | Post-node quiz questions are lazy-generated on first request for a skill and stored as a reusable quiz bank. The system generates exactly 8 multiple-choice questions (4 options, 1 correct answer) for the skill, returns exactly 5 per quiz attempt, and reuses the stored bank on subsequent attempts. This bank is **only** for post-node quizzes; onboarding assessment quizzes are generated by AI per goal and are not stored.                 |
...
Roadmap templates are publicly readable. Authenticated users can start learning directly on templates through the learner roadmap routes; template node progress is stored per user in `user_node_progress`. Admin template endpoints are reserved for template authoring and mutation only, including create/update/delete for templates and their nodes.
...
**Render deployment note:** The API and milestone evaluator are deployed as separate Render Docker web services from the root `render.yaml`. Each service must set `APP` so Render passes the same value as a Docker build arg and runtime environment variable, and each service must declare an explicit `dockerCommand` so only the intended process starts at runtime. The API service builds `docker/Dockerfile` with `APP=api`, runs `pnpm --filter api start:prod`, and exposes `/health` for Render readiness checks. The evaluator service builds the same Dockerfile with `APP=evaluator`, runs `pnpm --filter evaluator start:prod`, exposes `/health`, and must set `EVALUATOR_SHARED_SECRET` securely in Render. The API service must use `EVALUATOR_URL=<evaluator Render URL>` and the exact same `EVALUATOR_SHARED_SECRET` so signed milestone execution requests can be verified by the evaluator.
...
| **GROUP Node**        | A structural spine section that groups required/optional child skills; it is auto-completed by backend cascade logic and does not open the skill detail sidebar |
...
| **Timeline Warning**  | Automatic alert when the generated roadmap estimate exceeds available time or, after time has elapsed, actual learning speed is slower than planned             |
