    ...
    <section className="bg-background relative flex flex-col items-center overflow-hidden pt-20 pb-24 sm:pt-24 sm:pb-32 lg:pt-32">
      ...
      <SectionContainer className="relative z-10 flex flex-col items-center justify-center gap-16 lg:gap-32">
        ...
        >
          ...
          {/* Banner Text Left */}
          <div className="relative z-10 flex w-full max-w-136 flex-col items-center gap-8 text-center lg:shrink-0 lg:items-start lg:text-left">
            <div className="flex flex-col gap-6">
              <h2 className="text-title sm:text-3xl!">Personalized Roadmaps Powered by AI</h2>
        ...
        <div className="flex w-full flex-col-reverse items-center justify-between gap-16 lg:flex-row lg:items-center">
          ...
          <div className="relative flex w-full max-w-132 shrink-0 flex-col justify-center">
            ...
            <div
              className="pointer-events-none absolute top-0 left-8 h-[115%] w-0.5 bg-pink-300/20 sm:left-14"
              aria-hidden="true"
            />
            ...
            <ul className="flex w-full flex-col">
              {NOTEBOOK_ITEMS.map((item, i) => (
                <li
                  key={i}
                  className="relative flex min-h-14.5 items-center border-b border-indigo-200/60 py-3 transition-colors hover:bg-slate-50/50"
                >
                  {/* Icon Area placed to intersect the pink line */}
                  <div
                    className="bg-background absolute left-5.75 flex size-5 items-center justify-center rounded-full sm:left-11.75"
                    aria-hidden="true"
                  >
                    <HugeiconsIcon className={cn('size-4', item.iconClass)} icon={item.icon} />
                  </div>
                  {/* Text Container */}
                  <span className="text-foreground ml-16 text-base font-medium sm:ml-24 sm:text-lg">
                    {item.label}
                  </span>
                </li>
              ))}
